...
public class NetworkTests
{
    ...
    public void TestStopServer()
    {
        ...
        Assert.IsTrue(testListener.Server.IsBound);
        Networking.StopServer(testListener);
        Assert.IsFalse(testListener.Server.IsBound);
    }

    //ConnectToServer host name not valid

    [TestMethod]
    public void ConnectToServerInvalidHostname()
    {
        string errorMessage = "";
        bool errorOccured = false;
        void toCall(SocketState ss)
        {
            if (ss.ErrorOccurred)
            {
        ...
        Networking.ConnectToServer(toCall, "invalidHostName", 4000);

        Assert.IsTrue(errorOccured);
        Assert.IsTrue(errorMessage == "Invalid IP Address.");
    }

    [TestMethod]
    public void ConnectToServerInvalidHostnameIPV6()
    {
        string errorMessage = "";
        bool errorOccured = false;
        void toCall(SocketState ss)
        ...
        //use an ipv6 address instead of an ipv4
        Networking.ConnectToServer(toCall, "2601:681:6200:56a0:db5b:f82:26b1:6cf0", 4000);

        Assert.IsTrue(errorOccured);
        Assert.AreEqual("IPV4 address not found.", errorMessage);
    }

    //ConnectToServer cant Connect (BeginConnect)

    [TestMethod]
    public void ConnectToServerBeginConnectThrows()
    {
        string errorMessage = "";
        bool errorOccured = false;
        void toCall(SocketState ss)
        ...
        //use invalid port
        Networking.ConnectToServer(toCall, "localhost", 99999999);

        Assert.IsTrue(errorOccured);
        Assert.AreEqual("Error on Begin Connect", errorMessage);
    }
    ...
    [DataRow(false)]
    [DataTestMethod]
    public void TestSendAndCloseError(bool clientSide)
    {
        SetupTestConnections(clientSide, out testListener, out testLocalSocketState, out testRemoteSocketState);
        Assert.IsTrue(testLocalSocketState.TheSocket.Connected);
        testLocalSocketState.TheSocket.Shutdown(SocketShutdown.Both);
